...
df.rename(columns={'charges': 'EMI'}, inplace=True)
...
print("Bar plots for categorical Features Plot bar plot for the 'sex,smoker,region' feature.")
...
print("In the best case, they don't influence the model.In the worst case, they can cause the model to be overfit.")
...
# sns.boxplot(y='sex', x='EMI', data=df)
# plt.show()
# sns.boxplot(y='smoker', x='EMI', data=df)
# plt.show()
# sns.boxplot(y='region', x='EMI', data=df)
# plt.show()
print("----------------------------------------------------------------------------")
...
nan_rows = df[df['age'].isnull()].count()
print("no of nan_rows")
print(nan_rows)
print("*****************************************************************************")
...
df.smoker.replace('yes', 'Yes', inplace=True)
df.smoker.replace('no', 'No', inplace=True)
...
# sns.boxplot(df.EMI)
# plt.show()
# print(df.age.sort_values(ascending=False).head(20))
print("*****************************************************************************")
...
print("for the very practical reason that Scikit-Learn algorithms do not accept missing values here numerical feature")
...
df.to_csv('cleaned_insurance.csv', index=None)
...
with open('rfr_insurance.pkl', 'wb') as pickle_file:
       joblib.dump(win_model, 'rfr_insurance.pkl')

print("predicting the score")
pickle_model=joblib.load('rfr_insurance.pkl')
